...
uint8_t FDC1004_write(uint8_t reg, uint16_t data) {
   ...
   if (ret != 0) {
      Serial.printf("i2c error id: %d data: %d\n", ret, data);
   }
   return ret;
}
 ...
 * @return 0 if successful, 1 otherwise
 */
uint8_t FDC1004_configure_single_measurement(uint8_t measurement, uint8_t channel, uint8_t capdac) {
   ...
   */
   ...
   conf_data |= ((uint16_t)FDC1004_CAPDAC) << 10;
 ...
 * @return Status code
 */
uint8_t FDC1004_measure_channel(uint8_t channel, double *result, uint8_t *capdac) {
...
//            Serial.printf("too high capdac on channel %d: %d\n", channel, FDC1004_capdac_values[channel]);
         } else {
            return 1;
         }
      }
      ...
      if (error_code == 3) {
         if (FDC1004_capdac_values[channel] < 31) {
            FDC1004_capdac_values[channel] += 1;
//            Serial.printf("too low capdac on channel %d: %d\n", channel, FDC1004_capdac_values[channel]);
         } else {
            return 1;
         }
      }
      (*capdac) = FDC1004_capdac_values[channel];
   }
   return 0;
}
